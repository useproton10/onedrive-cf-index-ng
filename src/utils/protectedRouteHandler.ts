...
export function getStoredToken(path: string): string | null {
  const storedToken =
    typeof window !== 'undefined'
      ? JSON.parse(localStorage.getItem(matchProtectedRoute(path)) as string)
      : '';
  return storedToken ? encryptToken(storedToken) : null;
}

/**
 * Compares the hash of .password.txt and od-protected-token header
 * @param odTokenHeader od-protected-token header (sha256 hashed token)
 * @param dotPassword non-hashed .password.txt file content
